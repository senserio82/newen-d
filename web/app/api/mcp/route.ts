...
// Claude 커넥터 설정(Customize > Connectors > + > Add custom connector)에
...
const TOOLS = [
  ...
  },
  {
    name: "fetch_data",
    description:
      "키워드+기간에 매칭되는 실제 데이터 행을 가져옵니다. 가져온 건수만큼 newen.D 포인트가 1건당 1포인트 차감됩니다. max_rows 를 지정하지 않으면 매칭 건수와 보유 포인트 중 작은 값만큼 가져옵니다.",
    inputSchema: {
      type: "object",
      properties: {
        keyword: { type: "string" },
...
async function callTool(name: string, args: any, userId: string) {
  ...
  if (name === "fetch_data") {
    const { keyword, start_date, end_date, max_rows } = args ?? {};
    if (!keyword || !start_date || !end_date) {
      return errorResult("keyword, start_date, end_date are required");
    }
    try {
